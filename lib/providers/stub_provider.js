...
const StubLLMProvider = {
  ...
  async plan({ at, userGoal, visualContext } = {}) {
    ...
    if (lower.includes('laptop') || lower.includes('cheapest') || lower.includes('price') || lower.includes('lakh')) {
      // E-commerce price task — use real AT IDs so click/scroll resolves
