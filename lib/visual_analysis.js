...
// DOM-first: returns detailed object for flagging; caller in local_reasoning.js will FLAG needs_vlm instead of calling VLM until enabled.
function shouldTriggerFallback({ at, planResult } = {}) {
  if (!at || at.length === 0) return { needed: true, reason: 'AT empty' };
  if (planResult && planResult.reasoning && planResult.reasoning.toLowerCase().includes('at appears empty')) {
    return { needed: true, reason: 'LLM reported AT appears empty' };
  }
  if (at.length < 3 && planResult && planResult.confidence < 0.5) {
    return { needed: true, reason: `AT sparse (${at.length} nodes) with low confidence ${planResult.confidence}` };
  }
