...
if (typeof DEFAULT_THRESHOLD === 'undefined') { var DEFAULT_THRESHOLD = 0.7; }
...
async function reasonLocally({ at, userGoal, screenshot, threshold = DEFAULT_THRESHOLD, llm = null, vlm = null } = {}) {
  ...
  // Demo override: price/lakh tasks force cloud escalation to show Gemini even when stub confidence high (0.85)
  const goalLower = (userGoal || '').toLowerCase();
  const isPriceTask = /laptop|price|cheapest|lakh|under.*\d+/.test(goalLower);
  const effectiveThreshold = isPriceTask && threshold === DEFAULT_THRESHOLD ? 0.95 : threshold;
