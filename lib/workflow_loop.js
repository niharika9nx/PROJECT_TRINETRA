...
function isGoalAchieved({ at, userGoal, iteration, maxIterations = MAX_ITERATIONS } = {}) {
  // Stub evaluation: for testing, goal is "achieved" after 2–3 iterations or if AT contains product-like nodes
  // In real loop, cloud or local reasoning would signal completion; here we simulate deterministically.
  if (iteration >= 2) {
    // Check if AT has any node with price-like name — treat as product found
    if (at && at.some(n => /₹|price|rs\.?|laptop/i.test(n.name || ''))) return true;
    // Otherwise after 3 iterations treat as done for demo
...
async function runLocalLoop({
  ...
  while (iteration < maxIterations) {
    ...
    if (onStep) onStep({ step: 'reasoned', iteration, confidence: reasoning.confidence, needs_escalation: reasoning.needs_escalation, actions: reasoning.actions });
    ...
    // Step 5: Evaluate
    const achieved = isGoalAchieved({ at: observation.at, userGoal, iteration, maxIterations });
...
async function runFullLoop({
  ...
  serverUrl = 'http://localhost:3000/api/agent/act',
  onStep,
  maxIterations = MAX_ITERATIONS,
  threshold = DEFAULT_THRESHOLD
} = {}) {
  ...
  while (iteration < maxIterations) {
    ...
    if (onStep) onStep({ step: 'reason', iteration });
    const reasoning = await reason({ at: observation.at, userGoal, screenshot: observation.screenshot, threshold });
    history.push({ iteration, reasoning, observation, escalated: reasoning.needs_escalation });
    if (onStep) onStep({ step: 'reasoned', iteration, confidence: reasoning.confidence, needs_escalation: reasoning.needs_escalation, actions: reasoning.actions });
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { runLocalLoop, runFullLoop, observePage, isGoalAchieved, MAX_ITERATIONS, DEFAULT_THRESHOLD };
}
