...
function isGoalAchieved({ at, userGoal, iteration, maxIterations = MAX_ITERATIONS, history } = {}) {
  // Demo fix: don't fake success just because price exists — require actual click success or max iterations
  // Previously returned true after 2 iterations if any ₹ node, causing "Goal achieved" with 0 actions
  const lastExec = history && history.length > 0 ? history[history.length - 1]?.execResult : null;
  const hadSuccessfulClick = lastExec && lastExec.results && lastExec.results.some(r => r.result && r.result.success && (r.action?.type === 'click' || r.action?.type === 'scroll'));
  if (hadSuccessfulClick && iteration >= 2) {
    if (at && at.some(n => /₹|price|rs\.?|laptop/i.test(n.name || ''))) return true;
  }
  // Fallback: only after 5-6 iterations to allow real navigation/scroll to happen (demo needs movement)
  if (iteration >= 5) return true;
  // Also succeed if explicitly max reached
