...
function isGoalAchieved({ at, userGoal, iteration, maxIterations = MAX_ITERATIONS, history } = {}) {
  ...
  if (hadSuccessfulAction && iteration >= 2) {
    ...
    // Even without keyword, if we did a type action (search), consider success after 2
    if (iteration >= 2 && lastExec.results.some(r => r.action?.type === 'type')) return true;
...
// Detect if AT indicates a login/auth page (password fields, sign-in forms, etc.)
function detectAuthPage(at) {
  if (!Array.isArray(at) || at.length === 0) return { isAuth: false, reason: '' };
  const authKeywords = /sign\s*in|log\s*in|login|password|create\s*account|verify|otp|two.factor|authentication/i;
  for (const node of at) {
    const name = (node.name || '').toLowerCase();
    const tag = (node.tag || '').toLowerCase();
    const role = (node.role || '').toLowerCase();
    // Password-related name on any input/textbox
    if ((tag === 'input' || role === 'textbox') && authKeywords.test(name)) {
      return { isAuth: true, reason: `Auth-related element found: "${(node.name || '').slice(0, 60)}"` };
    }
    // Generic auth keyword match on any node name
    if (authKeywords.test(name) || authKeywords.test(role)) {
      return { isAuth: true, reason: `Auth-related element found: "${(node.name || '').slice(0, 60)}"` };
    }
  }
...
async function runFullLoop({
  ...
  reloadPageFn, // optional: () => void — auto-reload page after N stalled iterations
  ...
  while (iteration < maxIterations) {
    ...
    // Early exit: if goal involves checkout/payment but page shows login/auth, bail immediately
    if (goalNeedsAuth(userGoal)) {
    ...
    if (reasoning.needs_escalation) {
      ...
      try {
        ...
        // Cloud failed — fall back to local reasoning for this iteration
        console.log(`[Trinetra Loop] Iteration ${iteration}: cloud failed (${e.message}), falling back to local reasoning`);
        try {
          const fallbackReasoning = await reason({ at: observation.at, userGoal, screenshot: observation.screenshot, threshold });
    ...
    // Stall counter: track whether we took a valid action this iteration
    ...
    // Auto-reload after 3 stalled iterations to get fresh AT (e.g., search bar not in DOM)
